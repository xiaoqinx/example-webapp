...
pipeline {
    ...
    stages {
        ...
        stage('Deploy to Production fixed server') {
            ...
            steps {
                ...
                script {
                    ...
                    sh """
                       aws ec2 reboot-instances --region ap-southeast-2 --instance-ids i-0ecbf7d7381ccdf3d
                    """
                }
            }
        }
        ...
        stage('Deploy to Production') {
            ...
            steps {
                script {
                    PRODUCTION_ALB_LISTENER_ARN="arn:aws:elasticloadbalancing:ap-southeast-2:581251645584:listener/app/production-website/736542054cb9ffe8/b2c1e14a1b41024d"
                    sh """
